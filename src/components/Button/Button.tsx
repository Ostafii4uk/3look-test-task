...
const Button: React.FC<{type: ButtonType, clickFnc: Function, disabled?: boolean}> = ({type, clickFnc, disabled}) => {
  const getButtonText = () => {
    if (type === 'save') {
      return 'save changes'
    } else if (type === 'create') {
  ...
  return (
    <button className={`${Styles.button} ${Styles[type]} ${disabled && Styles.disabled }`} onClick={() => !disabled && clickFnc()}>
